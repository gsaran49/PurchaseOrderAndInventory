...
class kite_purchaseinvoice(osv.osv):

	_name = "kite.purchaseinvoice"
	_description = "Kite purchaseinvoice"
	...
	_columns = {
		...
		'line_id':fields.one2many('kite.purchaseinvoice.line','Item_Name','product line',readonly=True,states={'draft': [('readonly', False)]}),
	...
	def create(self, cr, uid, vals,context=None):		
		if vals.get('name','/')=='/':
			vals['name'] = self.pool.get('ir.sequence').get(cr, uid, 'kite.purchaseinvoice') or '/'
		order =  super(kite_purchaseinvoice, self).create(cr, uid, vals, context=context)
		return order
...
kite_purchaseinvoice()
class kite_purchaseinvoice_line(osv.osv):
	_name = "kite.purchaseinvoice.line"
	_description = "Kite purchaseinvoice line"
	...
	_columns = {
		...
		'product_line':fields.many2one('kite.purchaseinvoice','Product line'),
	}
