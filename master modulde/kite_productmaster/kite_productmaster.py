...
class kite_productmaster(osv.osv):

	_name = "kite.productmaster"
	_description = "Kite productmaster"
	...
	_columns = {
		...
		'line_id':fields.one2many('kite.productmaster.line','Item_Name','product line',readonly=True,states={'draft': [('readonly', False)]}),
	...
	def create(self, cr, uid, vals,context=None):		
		if vals.get('name','/')=='/':
			vals['name'] = self.pool.get('ir.sequence').get(cr, uid, 'kite.productmaster') or '/'
		order =  super(kite_productmaster, self).create(cr, uid, vals, context=context)
		return order
...
kite_productmaster()
class kite_productmaster_line(osv.osv):
	_name = "kite.customermaster.line"
	_description = "Kite productmaster line"
	...
	_columns = {
		...
		'product_line':fields.many2one('kite.productmaster','Product line'),
	}
