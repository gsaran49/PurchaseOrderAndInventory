...
class kite_countrymaster(osv.osv):

	_name = "kite.countrymaster"
	_description = "Kite countrymaster"

	
	_columns = {
		'Customer_name': fields.char('Customer name',size=128, select=True,required=True,readonly=True,states={'draft': [('readonly', False)]}),
		'Customer_country':fields.date('Customer_country',size=128, select=True,required=True,readonly=True,states={'draft': [('readonly', False)]}),
	...
	def create(self, cr, uid, vals,context=None):		
		if vals.get('name','/')=='/':
			vals['name'] = self.pool.get('ir.sequence').get(cr, uid, 'kite.countrymaster') or '/'
		order =  super(kite_countrymaster, self).create(cr, uid, vals, context=context)
		return order
...
kite_countrymaster()
class kite_countrymaster_line(osv.osv):
	_name = "kite.customermaster.line"
	_description = "Kite countrymaster line"
	...
	_columns = {
		...
		'product_line':fields.many2one('kite.countrymaster','Product line'),
	}
