...
class kite_productioninward(osv.osv):

	_name = "kite.productioninward"
	_description = "Kite productioninward"
	...
	_columns = {
		...
		'line_id':fields.one2many('kite.productioninward.line','Item_Name','product line',readonly=True,states={'draft': [('readonly', False)]}),
	...
	def create(self, cr, uid, vals,context=None):		
		if vals.get('name','/')=='/':
			vals['name'] = self.pool.get('ir.sequence').get(cr, uid, 'kite.productioninward') or '/'
		order =  super(kite_productioninward, self).create(cr, uid, vals, context=context)
		return order
...
kite_productioninward()
class kite_productioninward_line(osv.osv):
	_name = "kite.productioninward.line"
	_description = "Kite productioninward line"
	...
	_columns = {
		...
		'product_line':fields.many2one('kite.productioninward','Product line'),
	}
